...
namespace Lockstep
{
    ...
    public struct Vector2d : ICommandData
    {
        ...
        public void Rotate(long angle)
        {
            var cos = FixedMath.Trig.CosDegree(angle);
            var sin = FixedMath.Trig.SinDegree(angle);
            Rotate(cos, sin);
        }
        public void RotateInverse(long angle)
        {
            var cos = FixedMath.Trig.CosDegree(angle);
            var sin = FixedMath.Trig.SinDegree(angle);
            RotateInverse(cos, sin);
        }
