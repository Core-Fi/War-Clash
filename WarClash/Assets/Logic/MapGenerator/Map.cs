   ...
   public class Map : IMap
   {
      ...
      public void Restore( MapState state )
      {
         ...
         foreach ( Cell cell in GetAllCells() )
         {
            ...
            if (HasFlag((int)cellProperties, (int)(MapState.CellProperties.Visible)))
            {
               inFov.Add( IndexFor( cell.X, cell.Y ) );
            }
                _isTransparent[cell.X, cell.Y] = HasFlag((int)cellProperties, (int)MapState.CellProperties.Transparent);// cellProperties.HasFlag( MapState.CellProperties.Transparent );
                _isWalkable[cell.X, cell.Y] = HasFlag((int)cellProperties, (int)MapState.CellProperties.Walkable);//cellProperties.HasFlag( MapState.CellProperties.Walkable );
        }
