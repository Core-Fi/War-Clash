...
namespace RVO
{
    ...
    public class RVOTestQuadtree
    {
        ...
        struct Node
        {
            ...
            {
                UnityEngine.Vector2 c = r.center;

                while (linkedList != null)
                {
                    Agent nx = linkedList.next;
                    if (linkedList.position_.x_ > c.x)
                    {
                        if (linkedList.position_.y_ > c.y)
                        {
        ...
        public void Insert(Agent agent)
        {
            int i = 0;
            Rect r = bounds;
            Vector2 p = new Vector2(agent.position_.x_, agent.position_.y_);

            agent.next = null;

            maxRadius = System.Math.Max(agent.radius_, maxRadius);
        ...
        void DebugDrawRec(int i, Rect r)
        {
            ...
            while (a != null)
            {
                Debug.DrawLine(nodes[i].linkedList.position_.ToV3() + Vector3.up, a.position_.ToV3() + Vector3.up, new Color(1, 1, 0, 0.5f));
                a = a.next;
            }
        }
    }
